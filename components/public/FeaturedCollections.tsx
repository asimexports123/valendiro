...
import { PublicSubcategory } from "@/services/public/publicData";
import { SubcategoryIcon, CategoryBadge } from "@/components/public/CardVisual";

export function FeaturedSubcategories({ lang, subcategories }: { lang: string; subcategories: PublicSubcategory[] }) {
    ...
    <section className="border-y border-border/50 bg-muted/30 py-14 sm:py-16">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between mb-7">
          <div>
            <h2 className="text-xl sm:text-2xl font-semibold tracking-tight text-foreground">Popular Subcategories</h2>
            <p className="mt-1 text-sm text-muted-foreground">Curated learning paths — structured for deep learning</p>
          </div>
          <Link href={`/${lang}/subcategories`} className="text-sm font-medium text-primary hover:text-primary/80 transition shrink-0">
            View all →
          </Link>
        </div>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {subcategories.map((sub) => (
            ...
            >
              <SubcategoryIcon categorySlug={sub.category_slug} name={sub.name} />

              <div className="min-w-0 flex-1">
                <div className="mb-1.5">
                  <CategoryBadge slug={sub.category_slug} />
                </div>
