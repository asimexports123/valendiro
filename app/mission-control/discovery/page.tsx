...
export default function DiscoveryPage() {
  ...
  const fetchDiscoveryData = async () => {
    try {
      const res = await fetch("/api/mission-control/discovery/sources");
  ...
  const runSourceNow = async (sourceId: string, sourceType: string) => {
    try {
      await fetch(`/api/mission-control/discovery/sources/${sourceId}/run`, {
        method: "POST",
      });
      fetchDiscoveryData();
    } catch (error) {
  ...
  const toggleSource = async (sourceId: string, currentStatus: string) => {
    try {
      ...
      await fetch(`/api/mission-control/discovery/sources/${sourceId}`, {
        method: "PATCH",
