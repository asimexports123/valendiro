...
import { getCategoriesWithCounts, getSubcategoriesByCategory, SubcategoryDifficulty } from "@/services/public/publicData";
...
const SUBCATEGORY_ICONS: Record<string, string> = {
  ...
  "education-learning": "📖", "health-wellness": "💊", "home-lifestyle": "🛠️", travel: "🗺️",
};
...
export async function generateMetadata({
  ...
  return buildMetadata({
    title: "Subcategories — Valendiro",
    description: "Browse structured learning paths across 7 knowledge areas. Each subcategory groups related topics into a clear progression — from foundational concepts to advanced mastery.",
    canonical: `/${lang}/subcategories`,
  });
}
...
export default async function SubcategoriesPage({
  ...
  const subcategoriesPerCategory = await Promise.all(
    categories.map(async (cat) => ({
      category: cat,
      subcategories: await getSubcategoriesByCategory(cat.id, 20),
    }))
  );
  const grouped = subcategoriesPerCategory.filter((g) => g.subcategories.length > 0);
  const totalSubcategories = grouped.reduce((sum, g) => sum + g.subcategories.length, 0);
  const totalTopics = grouped.reduce((sum, g) => sum + g.subcategories.reduce((s, c) => s + c.topic_count, 0), 0);
      ...
      <div className="border-b border-border/50 bg-muted/20">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-14 sm:py-16">
          ...
          <p className="mt-3 text-base sm:text-lg text-muted-foreground max-w-2xl leading-relaxed">
            Structured learning paths organised by subject. Each subcategory groups related topics into a clear progression — from foundational concepts to advanced mastery.
          </p>
          ...
          {totalSubcategories > 0 && (
            <div className="mt-6 flex flex-wrap items-center gap-5">
              <div>
                <span className="text-2xl font-bold text-foreground">{totalSubcategories}</span>
                <span className="ml-1.5 text-sm text-muted-foreground">Subcategories</span>
              </div>
        ...
        {grouped.length === 0 ? (
          ...
          <div className="space-y-16">
            {grouped.map(({ category, subcategories }) => {
              ...
              const subcatIcon = SUBCATEGORY_ICONS[category.slug] ?? "📘";
              return (
                <section key={category.id}>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
                    {subcategories.map((sub) => {
                      ...
                      const hasContent = sub.topic_count > 0 || sub.article_count > 0;
                      return (
                        <Link
                          key={sub.id}
                          ...
                          <div className="mt-4 flex flex-wrap items-center gap-3 text-xs text-muted-foreground">
                            {sub.topic_count > 0 && (
                              <span className="flex items-center gap-1">
                                ...
                                <span><strong className="text-foreground/80">{sub.topic_count}</strong> topic{sub.topic_count !== 1 ? "s" : ""}</span>
                              </span>
                            )}
                            {sub.article_count > 0 && (
                              <span className="flex items-center gap-1">
                                ...
                                <span><strong className="text-foreground/80">{sub.article_count}</strong> article{sub.article_count !== 1 ? "s" : ""}</span>
                              </span>
                            )}
                            {sub.estimated_hours > 0 && hasContent && (
                              <span className="flex items-center gap-1">
                                <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
