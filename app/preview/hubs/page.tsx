...
export default async function PreviewHubsPage() {
  ...
  return (
    <div className="max-w-4xl mx-auto p-8 space-y-6">
      ...
      {hubs.length === 0 ? (
        <Card>
          <p className="text-center text-muted-foreground py-8">
            No Knowledge Hubs yet.
          </p>
        </Card>
      ) : (
        <div className="grid gap-4">
          {hubs.map((hub) => (
            <Link key={hub.id} href={`/preview/hubs/${hub.id}`}>
      ...
      <div className="text-center text-xs text-muted-foreground pt-4 border-t border-border/30">
        <Link href="/preview/entity-types" className="hover:text-foreground transition-colors">← View Entity Types</Link>
      </div>
    </div>
  );
}
