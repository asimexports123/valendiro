    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <Link href="/admin/entity-types/new">
          <Button>New Entity Type</Button>
        </Link>
      </div>

      {items.length === 0 ? (
        <Card>
          <p className="text-center text-muted-foreground py-8">No entity types yet. Create one to get started.</p>
        </Card>
      ) : (
        <div className="grid gap-4">
          {items.map((item) => (
            <Link key={item.id} href={`/admin/entity-types/${item.id}`}>
