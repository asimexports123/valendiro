...
export default function CEODashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="flex h-screen bg-[#050913] text-slate-100">
      <aside className="flex w-[252px] shrink-0 flex-col border-r border-white/[0.06] bg-gradient-to-b from-[#080f1d] to-[#060a12]">
        <div className="border-b border-white/[0.06] px-4 py-4">
          <div className="flex items-center gap-3">
            <div className="relative flex h-9 w-9 items-center justify-center rounded-xl bg-gradient-to-br from-violet-500 to-indigo-600 text-sm font-bold text-white shadow-lg shadow-violet-900/40">
              V
              <span className="absolute -right-0.5 -top-0.5 h-2 w-2 rounded-full border border-[#080f1d] bg-emerald-400" />
            </div>
            <div>
              <div className="text-[15px] font-semibold tracking-tight text-white">Valendiro</div>
              <div className="text-[10px] tracking-wide text-slate-500">Knowledge OS · Admin</div>
            </div>
          </div>
        </div>
                ...
                {section.items.map((item) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <item.icon
                        className={cn("h-3.5 w-3.5 shrink-0", active ? "text-violet-300" : "text-slate-500 group-hover:text-slate-400")}
                      />
                      {item.name}
                    </Link>
                  );
