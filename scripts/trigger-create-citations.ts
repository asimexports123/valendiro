...
async function triggerCreateCitations() {
  console.log("Creating citations for knowledge packages...\n");

  try {
    const response = await fetch(`${BASE_URL}/api/admin/legacy-create-citations`, {
      method: "POST",
      headers: {
    ...
    console.log("=== CITATION CREATION RESULTS ===\n");
    ...
    data.created.forEach((c: any) => {
      ...
      if (c.citation_id) {
        console.log(`  Citation ID: ${c.citation_id}`);
      }
      if (c.error) {
