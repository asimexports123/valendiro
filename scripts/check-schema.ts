/**
 * Check actual schema of topic_translations
 */

process.env.NEXT_PUBLIC_SUPABASE_URL = "https://diwwvkbztvhwouttajha.supabase.co";
process.env.SUPABASE_SERVICE_ROLE_KEY = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImRpd3d2a2J6dHZod291dHRhamhhIiwicm9sZSI6InNlcnZpY2Vfcm9sZSIsImlhdCI6MTc4MjY3NzIwMywiZXhwIjoyMDk4MjUzMjAzfQ.H-H9Ozpnn0M4d65ybDHOMVBQiK-CQFC9OPQPXq2b6yY";

import { createClient } from "@supabase/supabase-js";

const supabase = createClient(
  process.env.NEXT_PUBLIC_SUPABASE_URL!,
  process.env.SUPABASE_SERVICE_ROLE_KEY!
);

async function main() {
  console.log("Schema Check");
  console.log("============\n");

  // Get one record to see columns
  const { data: translations, error } = await supabase
    .from("topic_translations")
    .select("*")
    .limit(1);

  if (error) {
    console.log("Error:", error);
    return;
  }

  if (translations && translations.length > 0) {
    console.log("Columns in topic_translations:");
    console.log(Object.keys(translations[0]));
    console.log("\nSample record:");
    console.log(JSON.stringify(translations[0], null, 2));
  } else {
    console.log("No data in topic_translations");
  }

  // Check topics schema
  const { data: topics } = await supabase
    .from("topics")
    .select("*")
    .limit(1);

  if (topics && topics.length > 0) {
    console.log("\nColumns in topics:");
    console.log(Object.keys(topics[0]));
    console.log("\nSample record:");
    console.log(JSON.stringify(topics[0], null, 2));
  }
}

main().catch(console.error);
