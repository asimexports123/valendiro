/**
 * Apply Discovery System Migration Using Working Connection
 * Uses the same connection method as existing Knowledge OS
 */
...
async function applyMigration() {
  ...
  console.log("Applying Discovery System migration using working connection...");
  ...
  try {
    ...
    if (error) {
      console.log("exec_sql not available, trying alternate approach...");
      
      // Try using the database directly via PostgREST
      ...
      if (!response.ok) {
        console.log("RPC not available, creating tables via direct SQL execution...");
        await createTablesViaDirectSQL(supabase);
      } else {
...
async function createTablesViaDirectSQL(supabase: any) {
  ...
  for (const tableSQL of tables) {
    try {
      ...
      console.log(`Table creation: ${response.ok ? '✓' : '⚠'}`);
    } catch (e) {
